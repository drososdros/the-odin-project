...
function containUppercase(str: string): boolean {
  return /[A-Z]/.test(str)
}
function containNumber(str: string): boolean {
  return /[0-9]/.test(str)
}
function containSymbol(str: string): boolean {
  return /[@!/*#]/.test(str)
}
function validatePassword(str: string): boolean {
  let validateValues: boolean[] = [false, false, false, false]
